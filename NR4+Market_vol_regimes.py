...
df_daily['Min_Range_4'] = df_daily['Daily_Range'].rolling(window=4, min_periods=4).min()
df_daily['is_NR4_day'] = (df_daily['Daily_Range'] == df_daily['Min_Range_4'])

# Si 'is_NR4_day' est True pour le 19 juin, 'NR4_Signal_Available' sera True pour le 20 juin.
df_daily['NR4_Signal_Available'] = df_daily['is_NR4_day'].shift(1)

# Si le 19 juin était NR4, on a besoin de son High et Low pour un breakout le 20 juin.
df_daily['NR4_Prev_Day_High'] = df_daily['high'].shift(1)
df_daily['NR4_Prev_Day_Low'] = df_daily['low'].shift(1)

# on suppose qu'il n'y a pas de signal NR4 disponible.
df_daily['NR4_Signal_Available'].fillna(False, inplace=True)


################Pour eviter les NaN####################
#Si High est [10, NaN, 12, 15], après ffill, ça devient [10, 10, 12, 15]
df_daily['NR4_Prev_Day_High'].fillna(method='ffill', inplace=True)
df_daily['NR4_Prev_Day_Low'].fillna(method='ffill', inplace=True)
...
df_daily['NR4_Prev_Day_High'].fillna(df_daily['high'].iloc[0], inplace=True)
df_daily['NR4_Prev_Day_Low'].fillna(df_daily['low'].iloc[0], inplace=True)

''' 
fichier_performance = "strategy_metrics.txt"

with open(fichier_performance, 'a') as f:
    string_stats = df_daily['NR4_Signal_Available'].to_string()
    f.write("---New NR4+Market_vol_regimes---\n")
...
for d in range(1,len(all_days)):
    ...
    if prev_day in daily_groups.groups and current_day in daily_groups.groups:
        ...
        is_nr4_signal_today = df_daily.loc[date_as_timestamp, 'NR4_Signal_Available']
        nr4_prev_high = df_daily.loc[date_as_timestamp, 'NR4_Prev_Day_High']
        nr4_prev_low = df_daily.loc[date_as_timestamp, 'NR4_Prev_Day_Low']
        ...
        if is_nr4_signal_today:
            ...
            #nr4_buy_breakout_condition = (current_close_prices > nr4_prev_high) & \
                                         #(current_day_data['open'] < nr4_prev_high)
            ...
            #nr4_sell_breakout_condition = (current_close_prices < nr4_prev_low) & \
            #                              (current_day_data['open'] > nr4_prev_low)
            ...
            #signals[nr4_buy_breakout_condition] = 1
            #signals[nr4_sell_breakout_condition] = -1
