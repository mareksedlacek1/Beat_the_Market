...
df['day'] = pd.to_datetime(df['caldt'],
                           errors='coerce').dt.date  # Extract the date part from the datetime for daily analysis.
...
df = df.reset_index().merge(dividends[['day', 'dividend']], on='day', how='left').set_index(
    'caldt')  # df = df.merge(dividends[['day', 'dividend']], on='day', how='left')
...
df_daily['Min_Range_7'] = df_daily['Daily_Range'].rolling(window=7, min_periods=7).min()
df_daily['is_NR7_day'] = (df_daily['Daily_Range'] == df_daily['Min_Range_7'])

# Si 'is_NR4_day' est True pour le 19 juin, 'NR4_Signal_Available' sera True pour le 20 juin.
df_daily['NR7_Signal_Available'] = df_daily['is_NR7_day'].shift(1)

# Si le 19 juin était NR4, on a besoin de son High et Low pour un breakout le 20 juin.
df_daily['NR7_Prev_Day_High'] = df_daily['high'].shift(1)
df_daily['NR7_Prev_Day_Low'] = df_daily['low'].shift(1)

# on suppose qu'il n'y a pas de signal NR4 disponible.
df_daily['NR7_Signal_Available'].fillna(False, inplace=True)

################Pour eviter les NaN####################
# Si High est [10, NaN, 12, 15], après ffill, ça devient [10, 10, 12, 15]
df_daily['NR7_Prev_Day_High'].fillna(method='ffill', inplace=True)
df_daily['NR7_Prev_Day_Low'].fillna(method='ffill', inplace=True)
# S'il y a toujours des NaN au tout début après le ffill (parce que les premières valeurs sont NaN)
# on rempli avec la première valeur réelle de High/Low
df_daily['NR7_Prev_Day_High'].fillna(df_daily['high'].iloc[0], inplace=True)
df_daily['NR7_Prev_Day_Low'].fillna(df_daily['low'].iloc[0], inplace=True)
...
for d in range(1, len(all_days)):
    ...
    if prev_day in daily_groups.groups and current_day in daily_groups.groups:
        ...
        is_nr7_signal_today = df_daily.loc[date_as_timestamp, 'NR7_Signal_Available']
        nr7_prev_high = df_daily.loc[date_as_timestamp, 'NR7_Prev_Day_High']
        nr7_prev_low = df_daily.loc[date_as_timestamp, 'NR7_Prev_Day_Low']
        ...
        if is_nr7_signal_today:
            ...
            #    signals[(band_long_conditions & nr4_buy_breakout_condition)] = 1
            #    signals[(band_short_conditions & nr4_sell_breakout_condition)] = -1
            #    (Dans ce cas, tu devrais initialiser 'signals' à np.zeros_like(..) et ensuite appliquer cela)

            # B. NR4 comme OU un signal existant (plus de trades):
            #    signals[(band_long_conditions) | (nr4_buy_breakout_condition)] = 1
            #    signals[(band_short_conditions) | (nr4_sell_breakout_condition)] = -1
            #    (Ceci est conceptuellement proche de l'option que j'ai implémentée ci-dessus,
            #     mais l'implémentation directe par assignation peut gérer les priorités différemment.)
...
vix_performance['Total_Compounded_Return'] = results_df.groupby('VIX_Bucket')['Daily_Return'].apply(
...
with open(fichier_performance, 'a') as f:
    ...
    f.write("---New NR7+Market_vol_regimes---\n")
